...
async function main() {
  ...
  await prisma.restaurantOrder.upsert({ where: { companyId_locationId_code: { companyId: company.id, locationId: location.id, code: "ORD-DEMO-REST" } }, update: {}, create: { companyId: company.id, locationId: location.id, code: "ORD-DEMO-REST", tableId: restaurantTables[1].id, partnerId: demoCustomer.id, guestCount: 2, createdById: user.id, updatedById: user.id, lines: { create: { locationId: location.id, itemId: recipe.id, productName: recipe.name, baseUnitPrice: 8, quantity: 2, unitPrice: 8, vatRateId: vatRates.get("IVA10") ?? vatRates.get("IVA22")!, vatName: "IVA 10%", vatPercentage: 10, lineTotal: 16 } } } });
