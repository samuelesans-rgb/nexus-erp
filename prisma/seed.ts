...
async function main() {
  ...
  await prisma.restaurantBookingSettings.upsert({ where: { companyId_locationId: { companyId: company.id, locationId: location.id } }, update: {}, create: { companyId: company.id, locationId: location.id, openingHours: { "0": [["12:00", "23:00"]], "1": [["12:00", "15:00"], ["19:00", "23:00"]], "2": [["12:00", "15:00"], ["19:00", "23:00"]], "3": [["12:00", "15:00"], ["19:00", "23:00"]], "4": [["12:00", "15:00"], ["19:00", "23:00"]], "5": [["12:00", "15:00"], ["19:00", "23:00"]], "6": [["12:00", "23:00"]] }, minAdvanceMinutes: 0, defaultDurationMinutes: 120, slotIntervalMinutes: 30 } });
  ...
  const bankAccount = await prisma.financialAccount.upsert({ where: { companyId_code: { companyId: company.id, code: "BANCA" } }, update: { name: "Banca principale", active: true, deletedAt: null, updatedById: user.id }, create: { companyId: company.id, code: "BANCA", name: "Banca principale", type: "BANK", currency: "EUR", bankName: "Banca Demo", iban: "IT60X0542811101000000123456", openingBalance: 10000, createdById: user.id, updatedById: user.id } });
  const cashAccount = await prisma.financialAccount.upsert({ where: { companyId_code: { companyId: company.id, code: "CASSA" } }, update: { name: "Cassa principale", active: true, deletedAt: null, updatedById: user.id }, create: { companyId: company.id, code: "CASSA", name: "Cassa principale", type: "CASH", currency: "EUR", openingBalance: 500, createdById: user.id, updatedById: user.id } });
  await prisma.financialAccount.upsert({ where: { companyId_code: { companyId: company.id, code: "CARTA" } }, update: { name: "Carta aziendale", active: true, deletedAt: null, updatedById: user.id }, create: { companyId: company.id, code: "CARTA", name: "Carta aziendale", type: "CARD", currency: "EUR", openingBalance: 0, allowOverdraft: true, createdById: user.id, updatedById: user.id } });
  const dueDate = new Date(); dueDate.setDate(dueDate.getDate() + 30);
  const receivable = await prisma.paymentSchedule.upsert({ where: { companyId_documentId_installmentNumber: { companyId: company.id, documentId: demoInvoice.id, installmentNumber: 1 } }, update: {}, create: { companyId: company.id, partnerId: demoCustomer.id, documentId: demoInvoice.id, installmentNumber: 1, documentType: "SALES_INVOICE", direction: "RECEIVABLE", dueDate, amount: 36.6, residualAmount: 16.6, status: "PARTIALLY_PAID", currency: "EUR", paymentMethodId: defaultPaymentMethod.id, paymentTermId: defaultPaymentTerm.id, createdById: user.id, updatedById: user.id } });
  const payable = await prisma.paymentSchedule.upsert({ where: { companyId_documentId_installmentNumber: { companyId: company.id, documentId: demoPurchaseInvoice.id, installmentNumber: 1 } }, update: {}, create: { companyId: company.id, partnerId: demoSupplier.id, documentId: demoPurchaseInvoice.id, installmentNumber: 1, documentType: "PURCHASE_INVOICE", direction: "PAYABLE", dueDate, amount: 6.71, residualAmount: 0, status: "PAID", currency: "EUR", paymentMethodId: defaultPaymentMethod.id, paymentTermId: defaultPaymentTerm.id, createdById: user.id, updatedById: user.id } });
  const receiptMovement = await prisma.financialMovement.upsert({ where: { id: "demo-treasury-customer-receipt" }, update: {}, create: { id: "demo-treasury-customer-receipt", companyId: company.id, financialAccountId: bankAccount.id, partnerId: demoCustomer.id, paymentScheduleId: receivable.id, documentId: demoInvoice.id, movementType: "CUSTOMER_RECEIPT", direction: "IN", amount: 20, currency: "EUR", occurredAt: new Date(), reference: "INC-DEMO-001", postedById: user.id } });
  await prisma.financialAllocation.upsert({ where: { companyId_movementId_scheduleId: { companyId: company.id, movementId: receiptMovement.id, scheduleId: receivable.id } }, update: {}, create: { companyId: company.id, movementId: receiptMovement.id, scheduleId: receivable.id, amount: 20 } });
  const supplierPayment = await prisma.financialMovement.upsert({ where: { id: "demo-treasury-supplier-payment" }, update: {}, create: { id: "demo-treasury-supplier-payment", companyId: company.id, financialAccountId: bankAccount.id, partnerId: demoSupplier.id, paymentScheduleId: payable.id, documentId: demoPurchaseInvoice.id, movementType: "SUPPLIER_PAYMENT", direction: "OUT", amount: 6.71, currency: "EUR", occurredAt: new Date(), reference: "PAG-DEMO-001", postedById: user.id } });
  await prisma.financialAllocation.upsert({ where: { companyId_movementId_scheduleId: { companyId: company.id, movementId: supplierPayment.id, scheduleId: payable.id } }, update: {}, create: { companyId: company.id, movementId: supplierPayment.id, scheduleId: payable.id, amount: 6.71 } });
  const transfer = await prisma.financialTransfer.upsert({ where: { id: "demo-treasury-transfer" }, update: {}, create: { id: "demo-treasury-transfer", companyId: company.id, sourceAccountId: bankAccount.id, destinationAccountId: cashAccount.id, amount: 100, currency: "EUR", transferDate: new Date(), status: "COMPLETED", createdById: user.id, completedById: user.id, completedAt: new Date() } });
  ...
  await prisma.financialMovement.upsert({ where: { id: "demo-treasury-transfer-in" }, update: {}, create: { id: "demo-treasury-transfer-in", companyId: company.id, financialAccountId: cashAccount.id, movementType: "TRANSFER_IN", direction: "IN", amount: transfer.amount, currency: "EUR", occurredAt: transfer.transferDate, reference: `Transfer:${transfer.id}`, postedById: user.id } });
  const statementDate = new Date(); statementDate.setHours(0, 0, 0, 0);
  const statement = await prisma.bankStatement.upsert({ where: { companyId_financialAccountId_statementDate: { companyId: company.id, financialAccountId: bankAccount.id, statementDate } }, update: {}, create: { companyId: company.id, financialAccountId: bankAccount.id, statementDate, openingBalance: 10000, closingBalance: 9913.29, status: "RECONCILED" } });
  await prisma.bankStatementLine.upsert({ where: { id: "demo-treasury-statement-line" }, update: {}, create: { id: "demo-treasury-statement-line", companyId: company.id, bankStatementId: statement.id, transactionDate: new Date(), amount: 20, description: "Incasso cliente demo", reference: "INC-DEMO-001", matchedMovementId: receiptMovement.id, reconciliationStatus: "MATCHED" } });
