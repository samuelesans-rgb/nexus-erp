...
test("FUSION ORDER keeps byte-safe mapping, ACK ledger and uncertain delivery protection", async () => {
  ...
  const job = { jobId: "order", leaseToken: "lease", leaseExpiresAt: new Date().toISOString(), payload: "ignored", printType: "KITCHEN_TICKET", copies: 1, paperWidth: 80, printerType: "FUSION_XML_1745", connectionType: "TCP", fusionOrder: { tableIds: ["table"], lines: [{ lineId: "line", itemId: "item", plu: 3, quantity: 1, hasModifiers: false, hasNotes: false }] } };
  try { const adapter = new FusionXml1745PrinterAdapter(config(ackServer.address().port), new FusionDeliveryLedger(join(directory, "ack-ledger.json"))); await adapter.print(job); await adapter.print(job); assert.equal(payload, "<CE><ORDER><TABLE>199<PLU>3<MUL>1000</MUL></PLU></TABLE></ORDER></CE>"); } finally { await close(ackServer); }
  const timeoutServer = createTcpServer(socket => socket.on("data", () => undefined)); await listen(timeoutServer);
  try { const ledger = new FusionDeliveryLedger(join(directory, "uncertain-ledger.json")), adapter = new FusionXml1745PrinterAdapter(config(timeoutServer.address().port), ledger); await assert.rejects(adapter.print({ ...job, jobId: "uncertain" }), error => error instanceof FusionXml1745Error && error.outcome === "UNCERTAIN_DELIVERY"); assert.equal(await ledger.state("uncertain", "line"), "UNCERTAIN"); } finally { await close(timeoutServer); await rm(directory, { recursive: true }); }
});

test("catalog reader, snapshot, change detection, upload and manual version run end to end", async () => {
  const directory = await mkdtemp(join(tmpdir(), "nexus-standalone-catalog-")); let stage = 0; const acknowledgements = [];
  const fusion = createTcpServer(socket => socket.on("data", chunk => { const text = chunk.toString(); if (stage === 0 && text.includes("<DATA_REQ>")) { stage = 1; socket.write("<CE><DATA_SEND><PLU>2<DESC>CONTORNO</DESC><PRICE>400</PRICE></PLU></DATA_SEND></CE>"); } else if (text.includes("<ACK>")) { acknowledgements.push(text); if (stage === 1) { stage = 2; socket.write("<CE><DB_END/></CE>"); } else socket.end(); } })); await listen(fusion);
  ...
  try {
    ...
    controller.request(7); assert.equal(await controller.tick(true), true); assert.equal(uploads[0].status, "SYNCING"); assert.equal(uploads[1].requestVersion, 7); assert.deepEqual(uploads[1].items.map(item => item.plu), [2]); assert.equal(acknowledgements.length, 2);
