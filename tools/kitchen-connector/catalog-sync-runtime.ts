...
export class FusionCatalogSyncController {
  ...
  async tick(force=false){if(!this.config.enabled||this.running)return false;const now=Date.now(),backoff=Math.min(this.config.maxBackoffMs,this.config.intervalMs*2**this.failures);if(!force&&now-this.lastAttempt<backoff&&this.failures)return false;if(!force&&now-this.lastAttempt<this.config.intervalMs&&now-this.lastFull<this.config.fullIntervalMs)return false;
    this.running=true;this.lastAttempt=now;const runId=randomUUID(),watchdogMs=this.config.watchdogMs??300_000,watchdog=new AbortController(),timer=setTimeout(()=>watchdog.abort(new Error("CATALOG_SYNC_WATCHDOG_TIMEOUT")),watchdogMs);let completed=false,failure:unknown;
    try{await this.client.syncCatalog({status:"SYNCING",runId,watchdogMs},watchdog.signal);const previous=await this.store.load(),current=await this.reader.read(watchdog.signal),diff=reconcileCatalog(previous,current),signature=createHash("sha256").update(JSON.stringify([this.requestVersion,current.map(item=>item.rawFingerprint),diff.missing])).digest("hex");await this.client.syncCatalog({runId,idempotencyKey:`catalog:${runId}`,catalogFingerprint:signature,requestVersion:this.requestVersion,totalCount:current.length,unchangedCount:diff.unchanged,placeholdersSkipped:diff.placeholdersSkipped,items:diff.changed.map(item=>({plu:item.plu,name:item.name,priceCents:item.priceCents,fingerprint:item.rawFingerprint})),missingPlus:diff.missing},watchdog.signal);for(const item of Object.values(diff.snapshot.items))if(item.syncState==="PENDING")item.syncState="SYNCED";await this.store.save(diff.snapshot);this.failures=0;this.lastFull=Date.now();completed=true;return true;}catch(error){failure=error;this.failures=Math.min(this.failures+1,8);throw error;}finally{clearTimeout(timer);if(!completed)await this.client.syncCatalog({status:"ERROR",runId,error:String(failure??"Catalog sync interrotto")}).catch(error=>console.error("[catalog-sync] Impossibile persistere ERROR",error));this.running=false;}}
}
