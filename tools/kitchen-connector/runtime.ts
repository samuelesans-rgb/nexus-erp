...
export class KitchenConnectorClient {
  ...
  private async request(path:string,init:RequestInit={}){const response=await fetch(`${this.baseUrl.replace(/\/$/,"")}/api/kitchen-connector/v1${path}`,{...init,headers:{authorization:`Bearer ${this.credential}`,"content-type":"application/json",...init.headers}});const body=await response.json() as Record<string,unknown>;if(!response.ok)throw new Error(`${response.status}:${String(body.error??"Connector request failed")}`);return body;}
  private post(path:string,body:unknown){return this.request(path,{method:"POST",body:JSON.stringify(body)});}
  async heartbeat(lastError?:string){const[spool,printer]=await Promise.all([this.spool.diagnostics(),this.printer.diagnostics()]);await this.post("/heartbeat",{printerOnline:printer.status==="READY",queueDepth:spool.activeFiles,failedJobs:spool.failedFiles,lastError,connectorVersion:"1.0.0",diagnostics:{spool,printer,runtime:process.version,platform:process.platform}});}
