...
export type FusionCatalogEmptySlot = { plu:number;rawFingerprint:string };
export type FusionCatalogReadResult = { items:FusionCatalogItem[];emptySlots:FusionCatalogEmptySlot[] };
...
export function parseFusionCatalogFrame(frame:string):{kind:"ITEM";item:FusionCatalogItem}|{kind:"EMPTY_SLOT";slot:FusionCatalogEmptySlot}|{kind:"END"}{
  ...
  if(!normalizeFusionName(rawName)){
    if(rawPrice!==undefined&&priceCents===0){const rawFingerprint=catalogFingerprint({plu,name:"",priceCents:0});return{kind:"EMPTY_SLOT",slot:{plu,rawFingerprint}};}
    invalidCatalogFrame("EMPTY_DESC",frame,"DESC vuota senza PRICE zero.");
  }
...
export class FusionCatalogReader {
  ...
  read(signal?:AbortSignal):Promise<FusionCatalogReadResult>{return new Promise((resolve,reject)=>{
    ...
    const finish=(error?:Error,result?:FusionCatalogReadResult)=>{if(settled)return;settled=true;clearTimeout(timer);signal?.removeEventListener("abort",onAbort);socket.destroy();if(error)reject(error);else resolve(result??{items:[],emptySlots:[]});};
    ...
    const records=new Map<number,{kind:"ITEM";item:FusionCatalogItem}|{kind:"EMPTY_SLOT";slot:FusionCatalogEmptySlot}>();
    ...
    socket.on("data",chunk=>{bytes+=chunk.length;if(bytes>c.maxFrameBytes*c.maxItems)return finish(new FusionCatalogError("LIMIT","Risposta catalogo troppo grande."));buffer+=chunk.toString("utf8");
      for(let end;(end=buffer.indexOf(FRAME_END))>=0;){const frame=buffer.slice(0,end+FRAME_END.length);buffer=buffer.slice(end+FRAME_END.length);let parsed;try{parsed=parseFusionCatalogFrame(frame);}catch(error){return finish(error as Error);}arm(c.readTimeoutMs);if(parsed.kind==="END"){const values=[...records.values()];socket.write(FUSION_CATALOG_ACK,error=>error?finish(new FusionCatalogError("CONNECTION","ACK DB_END fallito.")):finish(undefined,{items:values.filter((value):value is {kind:"ITEM";item:FusionCatalogItem}=>value.kind==="ITEM").map(value=>value.item).sort((a,b)=>a.plu-b.plu),emptySlots:values.filter((value):value is {kind:"EMPTY_SLOT";slot:FusionCatalogEmptySlot}=>value.kind==="EMPTY_SLOT").map(value=>value.slot).sort((a,b)=>a.plu-b.plu)}));return;}socket.write(FUSION_CATALOG_ACK);const record=parsed.kind==="ITEM"?parsed.item:parsed.slot;if(record.plu>c.upperBoundPlu)return finish(new FusionCatalogError("PROTOCOL","PLU oltre upper bound."));const old=records.get(record.plu),oldRecord=old&&(old.kind==="ITEM"?old.item:old.slot);if(oldRecord&&oldRecord.rawFingerprint!==record.rawFingerprint)return finish(new FusionCatalogError("PROTOCOL","DATA_SEND duplicato conflittuale."));records.set(record.plu,parsed);if(records.size>c.maxItems)return finish(new FusionCatalogError("LIMIT","Troppi PLU."));}
    });
...
export function reconcileCatalog(previous:CatalogSnapshot,current:FusionCatalogItem[],at=new Date().toISOString(),emptySlots:FusionCatalogEmptySlot[]=[]){
  ...
  return{snapshot:{version:1 as const,completedAt:at,items},changed,missing,unchanged,placeholdersSkipped,emptySlotsSkipped:emptySlots.length};
}
