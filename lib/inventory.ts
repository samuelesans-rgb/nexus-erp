...
export type MovementInput = {
  warehouseId: string; binId?: string | null; itemId: string; movementType: InventoryMovementType;
...
async function postWithTx(tx: Prisma.TransactionClient, companyId: string, userId: string, input: MovementInput & { reversalOfId?: string; forcedDirection?: 1 | -1 }) {
  ...
  const [warehouse, item, unit, bin, lot, serial] = await Promise.all([
    tx.warehouse.findFirst({ where: { id: input.warehouseId, companyId, active: true, deletedAt: null }, select: { id: true, allowNegativeStock: true } }),
    ...
    input.lotId ? tx.inventoryLot.findFirst({ where: { id: input.lotId, companyId, itemId: input.itemId, active: true }, select: { id: true, expirationDate: true } }) : null,
    input.serialId ? tx.inventorySerial.findFirst({ where: { id: input.serialId, companyId, itemId: input.itemId }, select: { id: true, status: true } }) : null,
  ]);
  ...
  const movement = await tx.inventoryMovement.create({ data: { companyId, warehouseId: warehouse.id, binId: bin?.id, itemId: item.id, movementType: input.movementType, quantity, direction: movementDirection, unitOfMeasureId: unit.id, lotId: lot?.id, serialId: serial?.id, unitCost: movementCost, totalCost: quantity * movementCost, referenceType: input.referenceType, referenceId: input.referenceId, reason: input.reason, note: input.note, occurredAt: input.occurredAt ?? postedAt, postedAt, postedById: userId, reversalOfId: input.reversalOfId }, select: { id: true } });
  await tx.stockBalance.upsert({ where: { companyId_warehouseId_itemId: { companyId, warehouseId: warehouse.id, itemId: item.id } }, create: { companyId, warehouseId: warehouse.id, itemId: item.id, quantity: newQuantity, averageCost, stockValue }, update: { quantity: newQuantity, averageCost, stockValue } });
...
export async function reverseInventoryMovement(companyId: string, userId: string, movementId: string, reason?: string) {
  return prisma.$transaction(async (tx) => {
    const original = await tx.inventoryMovement.findFirst({ where: { id: movementId, companyId }, select: { id: true, warehouseId: true, binId: true, itemId: true, quantity: true, direction: true, unitOfMeasureId: true, lotId: true, serialId: true, unitCost: true, reversedBy: { select: { id: true } } } });
    ...
    return postWithTx(tx, companyId, userId, { warehouseId: original.warehouseId, binId: original.binId, itemId: original.itemId, movementType: "REVERSAL", quantity: original.quantity.toString(), unitOfMeasureId: original.unitOfMeasureId, lotId: original.lotId, serialId: original.serialId, unitCost: Number(original.unitCost ?? 0), reason: reason ?? "Storno", reversalOfId: original.id, forcedDirection: original.direction === 1 ? -1 : 1 });
...
export async function createTransfer(companyId: string, userId: string, input: { code: string; sourceWarehouseId: string; destinationWarehouseId: string; sourceBinId?: string | null; destinationBinId?: string | null; lines: Array<{ itemId: string; quantity: number; unitOfMeasureId: string; lotId?: string | null; serialId?: string | null }> }) {
  ...
  return prisma.$transaction(async (tx) => {
    const warehouses = await tx.warehouse.count({ where: { companyId, id: { in: [input.sourceWarehouseId, input.destinationWarehouseId] }, active: true, deletedAt: null } });
    if (warehouses !== 2) throw new InventoryDomainError("Magazzini non validi per la Company.");
    ...
    return tx.inventoryTransfer.create({ data: { companyId, code: input.code, sourceWarehouseId: input.sourceWarehouseId, destinationWarehouseId: input.destinationWarehouseId, sourceBinId: input.sourceBinId, destinationBinId: input.destinationBinId, createdById: userId, lines: { create: input.lines.map((line) => ({ ...line, quantity: positive(line.quantity, "La quantità") })) } }, select: { id: true } });
  });
}
...
export async function createInventoryCount(companyId: string, userId: string, input: { code: string; warehouseId: string; binId?: string | null; lines: Array<{ itemId: string; countedQuantity: number; unitOfMeasureId: string; lotId?: string | null; serialId?: string | null }> }) {
  if (!input.lines.length) throw new InventoryDomainError("L'inventario richiede almeno una riga.");
  return prisma.$transaction(async (tx) => {
    const warehouse = await tx.warehouse.findFirst({ where: { id: input.warehouseId, companyId, active: true, deletedAt: null }, select: { id: true } });
    if (!warehouse) throw new InventoryDomainError("Magazzino non valido per la Company.");
    ...
    for (const line of input.lines) {
      ...
      const movements = await tx.inventoryMovement.findMany({ where: { companyId, warehouseId: input.warehouseId, binId: input.binId || undefined, itemId: line.itemId, lotId: line.lotId || undefined, serialId: line.serialId || undefined }, select: { quantity: true, direction: true } });
    ...
    return tx.inventoryCount.create({ data: { companyId, code: input.code, warehouseId: input.warehouseId, binId: input.binId, createdById: userId, countedAt: new Date(), lines: { create: lines } }, select: { id: true } });
  });
}

export async function postInventoryCount(companyId: string, userId: string, countId: string) {
  return prisma.$transaction(async (tx) => {
    const count = await tx.inventoryCount.findFirst({ where: { id: countId, companyId, status: "DRAFT" }, include: { lines: true } });
...
export async function getStockBalance(companyId: string) { return prisma.stockBalance.findMany({ where: { companyId }, select: { quantity: true, averageCost: true, stockValue: true, item: { select: { id: true, code: true, name: true } }, warehouse: { select: { id: true, code: true, name: true } } }, orderBy: [{ item: { name: "asc" } }, { warehouse: { name: "asc" } }] }); }
export async function getItemStockByWarehouse(companyId: string, itemId: string) { return prisma.stockBalance.findMany({ where: { companyId, itemId }, select: { quantity: true, averageCost: true, stockValue: true, warehouse: { select: { id: true, code: true, name: true } } } }); }
export async function getLowStockItems(companyId: string) { const balances = await getStockBalance(companyId); const profiles = await prisma.productProfile.findMany({ where: { companyId, OR: [{ minimumStock: { not: null } }, { reorderPoint: { not: null } }] }, select: { itemId: true, minimumStock: true, reorderPoint: true } }); const thresholds = new Map(profiles.map((p) => [p.itemId, Number(p.minimumStock ?? p.reorderPoint ?? 0)])); return balances.filter((row) => Number(row.quantity) < (thresholds.get(row.item.id) ?? 0)); }
export async function getExpiringLots(companyId: string, days = 30) { const until = new Date(); until.setDate(until.getDate() + days); return prisma.inventoryLot.findMany({ where: { companyId, active: true, expirationDate: { gte: new Date(), lte: until } }, select: { id: true, lotNumber: true, expirationDate: true, item: { select: { id: true, code: true, name: true } }, movements: { select: { quantity: true, direction: true } } }, orderBy: { expirationDate: "asc" } }); }

export async function getInventoryOptions(companyId: string) {
  const [warehouses, items, lots, serials] = await Promise.all([
    prisma.warehouse.findMany({ where: { companyId, active: true, deletedAt: null }, select: { id: true, code: true, name: true, bins: { where: { active: true, deletedAt: null }, select: { id: true, code: true, name: true } } }, orderBy: { code: "asc" } }),
    ...
    prisma.inventoryLot.findMany({ where: { companyId, active: true }, select: { id: true, itemId: true, lotNumber: true, expirationDate: true }, orderBy: { lotNumber: "asc" } }),
    prisma.inventorySerial.findMany({ where: { companyId }, select: { id: true, itemId: true, serialNumber: true, status: true }, orderBy: { serialNumber: "asc" } }),
  ]);
  return { warehouses, items, lots, serials };
}

export async function getInventoryMovements(companyId: string, filters: { query?: string; movementType?: InventoryMovementType; warehouseId?: string; itemId?: string; from?: Date; to?: Date } = {}, page = 1) {
  const where: Prisma.InventoryMovementWhereInput = { companyId, movementType: filters.movementType, warehouseId: filters.warehouseId, itemId: filters.itemId, occurredAt: filters.from || filters.to ? { gte: filters.from, lte: filters.to } : undefined, ...(filters.query ? { OR: [{ item: { name: { contains: filters.query, mode: "insensitive" } } }, { item: { code: { contains: filters.query, mode: "insensitive" } } }, { referenceId: { contains: filters.query, mode: "insensitive" } }] } : {}) };
...
export async function getInventoryMovement(companyId: string, id: string) { return prisma.inventoryMovement.findFirst({ where: { id, companyId }, include: { item: { select: { code: true, name: true } }, warehouse: { select: { code: true, name: true } }, bin: { select: { code: true, name: true } }, lot: { select: { lotNumber: true, expirationDate: true } }, serial: { select: { serialNumber: true } }, postedBy: { select: { firstName: true, lastName: true, email: true } }, reversalOf: { select: { id: true } }, reversedBy: { select: { id: true } } } }); }

export async function getInventoryTransfers(companyId: string) { return prisma.inventoryTransfer.findMany({ where: { companyId }, include: { sourceWarehouse: { select: { code: true } }, destinationWarehouse: { select: { code: true } }, lines: { select: { id: true } } }, orderBy: { createdAt: "desc" } }); }
export async function getInventoryTransfer(companyId: string, id: string) { return prisma.inventoryTransfer.findFirst({ where: { id, companyId }, include: { sourceWarehouse: true, destinationWarehouse: true, sourceBin: true, destinationBin: true, lines: { include: { item: true, lot: true, serial: true, unitOfMeasure: true } } } }); }
export async function getInventoryCounts(companyId: string) { return prisma.inventoryCount.findMany({ where: { companyId }, include: { warehouse: { select: { code: true, name: true } }, lines: { select: { id: true } } }, orderBy: { createdAt: "desc" } }); }
export async function getInventoryCount(companyId: string, id: string) { return prisma.inventoryCount.findFirst({ where: { id, companyId }, include: { warehouse: true, bin: true, lines: { include: { item: true, lot: true, serial: true, unitOfMeasure: true } } } }); }
export async function getWarehouses(companyId: string, includeDeleted = false) { return prisma.warehouse.findMany({ where: { companyId, ...(includeDeleted ? {} : { deletedAt: null }) }, include: { location: true, bins: { orderBy: { code: "asc" } } }, orderBy: { code: "asc" } }); }
export async function getInventoryLots(companyId: string, query?: string) { return prisma.inventoryLot.findMany({ where: { companyId, ...(query ? { OR: [{ lotNumber: { contains: query, mode: "insensitive" } }, { item: { name: { contains: query, mode: "insensitive" } } }] } : {}) }, include: { item: { select: { code: true, name: true } }, movements: { select: { quantity: true, direction: true } } }, orderBy: { expirationDate: { sort: "asc", nulls: "last" } } }); }
export async function getInventoryDashboard(companyId: string) { const [movements, transfers] = await Promise.all([prisma.inventoryMovement.findMany({ where: { companyId }, select: { id: true, movementType: true, quantity: true, direction: true, occurredAt: true, item: { select: { code: true } }, warehouse: { select: { code: true } } }, orderBy: { occurredAt: "desc" }, take: 6 }), prisma.inventoryTransfer.findMany({ where: { companyId, status: "DRAFT" }, select: { id: true, code: true, sourceWarehouse: { select: { code: true } }, destinationWarehouse: { select: { code: true } } }, orderBy: { createdAt: "desc" }, take: 6 })]); return { movements, transfers }; }
export async function getDetailedStock(companyId: string) { const movements = await prisma.inventoryMovement.findMany({ where: { companyId }, select: { quantity: true, direction: true, warehouse: { select: { code: true } }, bin: { select: { code: true } }, item: { select: { code: true, name: true } }, lot: { select: { lotNumber: true } }, serial: { select: { serialNumber: true } } } }); const grouped = new Map<string, { warehouse: string; bin: string; item: string; lot: string; serial: string; quantity: number }>(); for (const row of movements) { const data = { warehouse: row.warehouse.code, bin: row.bin?.code ?? "—", item: `${row.item.code} · ${row.item.name}`, lot: row.lot?.lotNumber ?? "—", serial: row.serial?.serialNumber ?? "—", quantity: 0 }; const key = JSON.stringify(data); const current = grouped.get(key) ?? data; current.quantity += Number(row.quantity) * row.direction; grouped.set(key, current); } return [...grouped.values()].filter((row) => row.quantity !== 0).sort((a, b) => a.item.localeCompare(b.item)); }
