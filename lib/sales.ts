...
async function seriesFor(companyId: string, documentType: DocumentType) {
  const series = await prisma.documentSeries.findFirst({ where: { companyId, documentType, active: true }, select: { id: true }, orderBy: { code: "asc" } });
...
export async function createQuote(companyId: string, userId: string, input: Omit<DraftInput, "seriesId"> & { seriesId?: string }) {
  const seriesId = input.seriesId ?? await seriesFor(companyId, "QUOTE");
  const validSeries = await prisma.documentSeries.count({ where: { id: seriesId, companyId, documentType: "QUOTE", active: true } });
...
export async function convertDocument(companyId: string, userId: string, sourceDocumentId: string, targetType: DocumentType) {
  const source = await prisma.businessDocument.findFirst({ where: { id: sourceDocumentId, companyId, deletedAt: null }, include: { lines: { orderBy: { lineNumber: "asc" } } } });
  ...
  const target = await createDraft(companyId, userId, {
    seriesId: await seriesFor(companyId, targetType), partnerId: source.partnerId, documentDate: new Date(), currency: source.currency,
    exchangeRate: Number(source.exchangeRate), warehouseId: source.warehouseId, locationId: source.locationId,
...
export const createOrderFromQuote = (companyId: string, userId: string, id: string) => convertDocument(companyId, userId, id, "SALES_ORDER");
export const createDeliveryFromOrder = (companyId: string, userId: string, id: string) => convertDocument(companyId, userId, id, "DELIVERY_NOTE");
export const createInvoiceFromDelivery = (companyId: string, userId: string, id: string) => convertDocument(companyId, userId, id, "SALES_INVOICE");
export const createInvoiceFromOrder = (companyId: string, userId: string, id: string) => convertDocument(companyId, userId, id, "SALES_INVOICE");
export async function duplicateQuote(companyId: string, userId: string, id: string) {
  const source = await prisma.businessDocument.findFirst({ where: { id, companyId, documentType: "QUOTE", deletedAt: null }, select: { id: true } });
  if (!source) throw new SalesDomainError("Preventivo da duplicare non trovato.");
  const copy = await duplicateDraft(companyId, userId, id); await emit(companyId, "QuoteCreated", copy.id, { duplicatedFromId: id }); return copy;
}

export async function confirmSalesOrder(companyId: string, userId: string, id: string) {
  const order = await prisma.businessDocument.findFirst({ where: { id, companyId, documentType: "SALES_ORDER", status: "DRAFT", deletedAt: null }, select: { id: true } });
  if (!order) throw new SalesDomainError("Solo un ordine Draft può essere confermato.");
  await confirmDocument(companyId, userId, id);
  await emit(companyId, "SalesOrderConfirmed", id);
  return { id };
}

export async function postDelivery(companyId: string, userId: string, id: string) {
  await requireModule(companyId, MODULE_CODES.CORE_INVENTORY);
  const delivery = await prisma.businessDocument.findFirst({ where: { id, companyId, documentType: "DELIVERY_NOTE", status: "CONFIRMED", deletedAt: null }, include: { lines: { include: { item: { select: { stockManaged: true } } } } } });
  ...
  await postDocument(companyId, userId, id);
  await emit(companyId, "DeliveryPosted", id);
  return { id };
}

export async function postInvoice(companyId: string, userId: string, id: string) {
  const invoice = await prisma.businessDocument.findFirst({ where: { id, companyId, documentType: "SALES_INVOICE", status: "CONFIRMED", deletedAt: null }, select: { id: true } });
...
export async function getSalesDocuments(companyId: string, filters: { query?: string; documentType?: DocumentType; status?: DocumentStatus; page?: number } = {}) {
  if (filters.documentType && (SALES_TYPES as readonly DocumentType[]).includes(filters.documentType)) return getDocuments(companyId, filters);
  const page = Math.max(filters.page ?? 1, 1); const where: Prisma.BusinessDocumentWhereInput = { companyId, deletedAt: null, documentType: { in: SALES_TYPES }, status: filters.status, ...(filters.query ? { OR: [{ documentNumber: { contains: filters.query, mode: "insensitive" } }, { partner: { name: { contains: filters.query, mode: "insensitive" } } }] } : {}) };
...
export async function getSalesDocument(companyId: string, id: string) {
...
export async function getSalesDashboard(companyId: string) {
  ...
  const [quotesOpen, ordersOpen, deliveriesToInvoice, invoicesMonth, orderValue, quotes, converted] = await Promise.all([
    prisma.businessDocument.count({ where: { companyId, documentType: "QUOTE", status: { in: ["DRAFT", "CONFIRMED"] }, deletedAt: null } }),
    prisma.businessDocument.count({ where: { companyId, documentType: "SALES_ORDER", status: { in: ["DRAFT", "CONFIRMED"] }, deletedAt: null } }),
    prisma.businessDocument.count({ where: { companyId, documentType: "DELIVERY_NOTE", status: "POSTED", sourceLinks: { none: { linkType: "DDT_TO_INVOICE" } }, deletedAt: null } }),
    prisma.businessDocument.aggregate({ where: { companyId, documentType: "SALES_INVOICE", documentDate: { gte: start }, deletedAt: null }, _sum: { total: true }, _count: true }),
    prisma.businessDocument.aggregate({ where: { companyId, documentType: "SALES_ORDER", status: { in: ["DRAFT", "CONFIRMED"] }, deletedAt: null }, _sum: { total: true } }),
    prisma.businessDocument.count({ where: { companyId, documentType: "QUOTE", deletedAt: null } }),
    prisma.documentLink.count({ where: { companyId, linkType: "QUOTE_TO_ORDER" } }),
  ]);
