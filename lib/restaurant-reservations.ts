...
export async function createReservation(companyId: string, userId: string, input: { locationId: string; partnerId?: string|null; guestName: string; phone?: string; email?: string; startTime: Date; endTime?: Date|null; partySize: number; source: RestaurantReservationSource; status?: RestaurantReservationStatus; notes?: string; tableIds?: string[]; adminOverride?: boolean }) {
  if (!input.guestName.trim() || input.partySize < 1) throw new RestaurantDomainError("Ospite e numero coperti sono obbligatori.");
  const tables = await prisma.restaurantTable.findMany({ where: { companyId, locationId: input.locationId, id: { in: input.tableIds ?? [] }, active: true, deletedAt: null }, select: { id: true, seats: true, maxSeats: true, status: true } });
...
export async function transitionReservation(companyId: string, userId: string, id: string, status: RestaurantReservationStatus) {
  ...
  if (status === "SEATED") await prisma.restaurantTable.updateMany({ where:{companyId,id:{in:current.tables.map(t=>t.tableId)}},data:{status:"OCCUPIED"} });
...
export const getReservations = (companyId:string) => prisma.restaurantReservation.findMany({ where:{companyId,deletedAt:null},include:{location:true,tables:{include:{table:true}}},orderBy:{startTime:"asc"} });
