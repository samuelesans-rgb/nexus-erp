...
export async function salesOperationAction(data: FormData) {
  const { companyId, locationId, userId } = await requireSalesContext(true); const id = text(data, "id")!; const kind = text(data, "kind") ?? "quotes"; const operation = text(data, "operation"); let targetId = id; let targetKind = kind;
  try {
    if (operation === "confirm") { const type = salesRoute(kind)?.type; if (type === "SALES_ORDER") await confirmSalesOrder(companyId, userId, locationId, id); else await confirmDocument(companyId, userId, locationId, id); }
    else if (operation === "post-delivery") await postDelivery(companyId, userId, locationId, id);
    else if (operation === "post-invoice") await postInvoice(companyId, userId, locationId, id);
    else if (operation === "duplicate") { const copy = await duplicateQuote(companyId, userId, locationId, id); targetId = copy.id; }
    else { const targetType = operation === "to-order" ? "SALES_ORDER" : operation === "to-delivery" ? "DELIVERY_NOTE" : operation === "to-invoice" ? "SALES_INVOICE" : null; if (!targetType) throw new SalesDomainError("Operazione non valida."); const target = await convertDocument(companyId, userId, locationId, id, targetType); targetId = target.id; targetKind = kindForType(targetType); }
