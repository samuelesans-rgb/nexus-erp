...
import { getAssignableTables, getReservationHistory, getStaffReservation } from "@/lib/restaurant-booking";
...
const actionLabels: Partial<Record<RestaurantReservationStatus, string>> = { CONFIRMED: "Conferma", CANCELLED: "Annulla", SEATED: "Segna al tavolo", COMPLETED: "Completa", NO_SHOW: "Segna no-show" };
...
export default async function ReservationDetailPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ error?: string; success?: string }> }) {
  ...
  const [tables, history] = await Promise.all([getAssignableTables(context.companyId, location.id), getReservationHistory(context.companyId, id)]);
  ...
  return <div className="space-y-6">
    ...
    <section className="grid gap-4 rounded-xl border bg-white p-5 sm:grid-cols-2 lg:grid-cols-4" aria-labelledby="summary-title">
      ...
      <div><span className="text-xs uppercase text-slate-500">Tavolo</span><p className={currentTable ? "font-medium" : "font-medium text-amber-700"}>{currentTable ? `${currentTable.code} · ${currentTable.name}` : "Senza tavolo"}</p></div>
