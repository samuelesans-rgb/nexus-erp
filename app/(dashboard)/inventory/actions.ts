...
export async function saveWarehouse(formData: FormData) {
  ...
  if (id) await prisma.warehouse.updateMany({ where: { id, companyId, locationId: location.id }, data: { code, name, description: value(formData, "description"), allowNegativeStock: formData.get("allowNegativeStock") === "on", active: formData.get("active") === "on", updatedById: userId } });
  else await prisma.warehouse.create({ data: { companyId, locationId: location.id, code, name, description: value(formData, "description"), allowNegativeStock: formData.get("allowNegativeStock") === "on", createdById: userId, updatedById: userId } });
...
export async function setWarehouseLifecycle(formData: FormData) {
  const { companyId, userId } = await requireInventoryContext(); const location = await requireCurrentLocation(); const id = value(formData, "id"); const restore = formData.get("restore") === "true";
  if (id) await prisma.warehouse.updateMany({ where: { id, companyId, locationId: location.id }, data: { active: restore, deletedAt: restore ? null : new Date(), updatedById: userId } });
  revalidatePath("/inventory"); redirect("/inventory/warehouses");
}

export async function saveBin(formData: FormData) {
  const { companyId } = await requireInventoryContext(); const location = await requireCurrentLocation(); const warehouseId = value(formData, "warehouseId"); const code = value(formData, "code")?.toUpperCase(); const name = value(formData, "name");
  if (!warehouseId || !code || !name || !(await prisma.warehouse.findFirst({ where: { id: warehouseId, companyId, locationId: location.id, deletedAt: null }, select: { id: true } }))) redirect("/inventory/warehouses?error=Dati ubicazione non validi");
...
export async function reverseMovementAction(formData: FormData) { const { companyId, userId } = await requireInventoryContext(); const location = await requireCurrentLocation(); const id = value(formData, "id"); if (!id) redirect("/inventory/movements"); try { await reverseInventoryMovement(companyId, userId, id, value(formData, "reason") ?? undefined, location.id); revalidatePath("/inventory"); redirect(`/inventory/movements/${id}?success=Movimento stornato`); } catch (error) { redirect(`/inventory/movements/${id}?error=${encodeURIComponent(inventoryError(error))}`); } }

export async function createTransferAction(formData: FormData) { const { companyId, userId } = await requireInventoryContext(); const location = await requireCurrentLocation(); let id: string; try { const transfer = await createTransfer(companyId, userId, { code: value(formData, "code")!, sourceWarehouseId: value(formData, "sourceWarehouseId")!, destinationWarehouseId: value(formData, "destinationWarehouseId")!, sourceBinId: value(formData, "sourceBinId"), destinationBinId: value(formData, "destinationBinId"), lines: [{ itemId: value(formData, "itemId")!, quantity: number(formData, "quantity"), unitOfMeasureId: value(formData, "unitOfMeasureId")!, lotId: value(formData, "lotId"), serialId: value(formData, "serialId") }] }); id = transfer.id; } catch (error) { redirect(`/inventory/transfers/new?error=${encodeURIComponent(inventoryError(error))}`); } if (!id) redirect("/inventory/transfers/new?error=Trasferimento%20non%20valido"); const transfer = await prisma.inventoryTransfer.findFirst({ where: { id, companyId, sourceLocationId: location.id, destinationLocationId: location.id }, select: { id: true } }); if (!transfer) redirect("/inventory/transfers/new?error=Magazzino%20non%20valido%20per%20la%20Location%20corrente"); redirect(`/inventory/transfers/${id}`); }
export async function completeTransferAction(formData: FormData) { const { companyId, userId } = await requireInventoryContext(); const location = await requireCurrentLocation(); const id = value(formData, "id")!; try { const transfer = await prisma.inventoryTransfer.findFirst({ where: { id, companyId, sourceLocationId: location.id, destinationLocationId: location.id }, select: { id: true } }); if (!transfer) throw new InventoryDomainError("Trasferimento non valido per la Location corrente."); await completeTransfer(companyId, userId, id); revalidatePath("/inventory"); redirect(`/inventory/transfers/${id}?success=Trasferimento completato`); } catch (error) { redirect(`/inventory/transfers/${id}?error=${encodeURIComponent(inventoryError(error))}`); } }

export async function createCountAction(formData: FormData) { const { companyId, userId } = await requireInventoryContext(); const location = await requireCurrentLocation(); let id: string; try { const count = await createInventoryCount(companyId, userId, { code: value(formData, "code")!, warehouseId: value(formData, "warehouseId")!, locationId: location.id, binId: value(formData, "binId"), lines: [{ itemId: value(formData, "itemId")!, countedQuantity: number(formData, "countedQuantity"), unitOfMeasureId: value(formData, "unitOfMeasureId")!, lotId: value(formData, "lotId"), serialId: value(formData, "serialId") }] }); id = count.id; } catch (error) { redirect(`/inventory/counts/new?error=${encodeURIComponent(inventoryError(error))}`); } redirect(`/inventory/counts/${id}`); }
export async function postCountAction(formData: FormData) { const { companyId, userId } = await requireInventoryContext(); const location = await requireCurrentLocation(); const id = value(formData, "id")!; try { await postInventoryCount(companyId, userId, id, location.id); revalidatePath("/inventory"); redirect(`/inventory/counts/${id}?success=Inventario contabilizzato`); } catch (error) { redirect(`/inventory/counts/${id}?error=${encodeURIComponent(inventoryError(error))}`); } }
