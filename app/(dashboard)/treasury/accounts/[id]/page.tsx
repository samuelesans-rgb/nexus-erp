...
export default async function Page({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ success?: string }> }) {
  const { companyId, locationId } = await requireTreasuryContext(); const row = await getTreasuryAccount(companyId, locationId, (await params).id); if (!row) notFound(); const query = await searchParams;
