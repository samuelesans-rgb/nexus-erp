...
test("multi-table reservation -> order, conflict and release",async({page})=>{
 ...
 try{
  ...
  documentId=(await prisma.restaurantOrder.findUniqueOrThrow({where:{id:orderId},select:{documentId:true}})).documentId??"";expect((await prisma.restaurantTable.findMany({where:{id:{in:tableIds}},select:{physicalStatus:true}})).map(x=>x.physicalStatus)).toEqual(["DIRTY","DIRTY"]);
 }finally{
