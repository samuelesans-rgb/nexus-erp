...
before(async () => {
  ...
  const legacy = await prisma.documentSeries.create({ data: { companyId, code: `SP-L-${suffix}`, name: "Serie legacy", documentType: "QUOTE" } }); series.set("LEGACY_QUOTE", legacy.id); seriesIds.push(legacy.id);
...
test("Compatibilità 17: serie Documents globale legacy resta limitata all’adapter previsto", async () => { const row = await createQuote(companyId, userId, documentInput(customerId, locationA, warehouseA, series.get("LEGACY_QUOTE")!)); documentIds.push(row.id); assert.equal((await getSalesDocument(companyId, locationA, row.id))?.locationId, locationA); });
