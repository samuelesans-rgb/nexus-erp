...
test("Compatibilità 17: nessuna serie Documents globale resta disponibile", async () => { const rows=await prisma.$queryRaw<Array<{count:bigint}>>`SELECT count(*)::bigint AS count FROM "DocumentSeries" WHERE "companyId"=${companyId} AND "locationId" IS NULL`; assert.equal(Number(rows[0].count),0); });
