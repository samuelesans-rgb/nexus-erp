...
if (!databaseName.endsWith("_test"))
  throw new Error(
    "Kitchen Connector tests require a database ending in _test.",
  );
...
async function createFixture() {
  ...
  const company = await prisma.company.create({
    data: { name: `Connector ${suffix}`, vatNumber: `KC${suffix}` },
  });
  const user = await prisma.user.create({
    data: {
      email: `connector-${suffix}@test.invalid`,
      firstName: "Test",
      lastName: "Connector",
      password: "test",
    },
...
test("Kitchen Connector V1 integration and simulator lifecycle", async () => {
  ...
  const paired = await pairConnector(pairToken.pairingToken, {
    name: "simulator",
    serialConfig: { port: "virtual" },
  });
  await assert.rejects(
    pairConnector(pairToken.pairingToken, { name: "duplicate" }),
  );
  let device = await authenticateConnector(paired.credential);
  await heartbeatConnector(device.id, {
    printerOnline: true,
    queueDepth: 0,
    failedJobs: 0,
    diagnostics: { adapter: "SIMULATOR" },
  });
  const job = await createConnectorTestPrint(
    fixture.company.id,
    fixture.location.id,
    fixture.printer.id,
    fixture.user.id,
  ...
  assert.equal(
    (await fetchConnectorJobs(device)).some((row) => row.id === job.id),
    true,
  );
  const claims = await Promise.allSettled([
    claimConnectorJob(device, job.id),
    claimConnectorJob(device, job.id),
  ]);
  assert.equal(
  ...
  assert.equal(
    (await acknowledgeConnectorJob(device, job.id, claim.leaseToken)).status,
    "PRINTED",
  );
  assert.equal(
    (await acknowledgeConnectorJob(device, job.id, claim.leaseToken)).status,
  ...
  const failed = await createConnectorTestPrint(
    fixture.company.id,
    fixture.location.id,
    fixture.printer.id,
    fixture.user.id,
  ...
  await fetchConnectorJobs(device);
  await assert.rejects(claimConnectorJob(device, failed.id));
  assert.equal(
    (
      await prisma.kitchenPrintJob.findUniqueOrThrow({
        where: { id: failed.id },
