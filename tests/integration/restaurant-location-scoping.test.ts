...
let fixturePrinterId="",companyId="",otherCompanyId="",userId="",locationA="",locationB="",partnerId="",itemId="",recipeId="",seriesId="",accountId="",areaA="",areaB="",tableA="",tableB="",orderId="",lineId="";
...
before(async()=>{
  ...
  const fixtureAssignment=await prisma.kitchenStationAssignment.findFirstOrThrow({where:{companyId,active:true,station:{locationId:locationA,active:true},OR:[{itemId:recipeId},{itemCategoryId:recipe.categoryId}]},include:{station:true},orderBy:[{itemId:"desc"},{priority:"desc"}]});fixturePrinterId=(await prisma.restaurantPrinter.create({data:{companyId,locationId:locationA,stationId:fixtureAssignment.kitchenStationId,code:`TEST-${suffix}`,name:"Restaurant test mock"}})).id;
...
after(async()=>{
  ...
  if(orderIds.length){await prisma.idempotencyRecord.deleteMany({where:{aggregateId:{in:orderIds}}});const tickets=await prisma.kitchenTicket.findMany({where:{orderId:{in:orderIds}},select:{id:true}});if(tickets.length)await prisma.kitchenPrintJob.deleteMany({where:{ticketId:{in:tickets.map(x=>x.id)}}});if(tickets.length)await prisma.kitchenTicketLine.deleteMany({where:{ticketId:{in:tickets.map(x=>x.id)}}});if(tickets.length)await prisma.kitchenTicket.deleteMany({where:{id:{in:tickets.map(x=>x.id)}}});await prisma.kitchenDispatch.deleteMany({where:{orderId:{in:orderIds}}});await prisma.recipeConsumption.deleteMany({where:{orderId:{in:orderIds}}});await prisma.restaurantOrderLine.deleteMany({where:{orderId:{in:orderIds}}});await prisma.restaurantOrder.updateMany({where:{id:{in:orderIds}},data:{documentId:null}});await prisma.restaurantOrder.deleteMany({where:{id:{in:orderIds}}});}
  ...
  await prisma.auditLog.deleteMany({where:{companyId,entityType:{in:["KitchenDispatch","KitchenTicket"]}}});if(fixturePrinterId)await prisma.restaurantPrinter.delete({where:{id:fixturePrinterId}});await prisma.financialAccount.deleteMany({where:{id:accountId}});await prisma.documentSeries.deleteMany({where:{id:seriesId}});await prisma.warehouseBin.deleteMany({where:{warehouse:{locationId:{in:locationIds}}}});await prisma.warehouse.deleteMany({where:{locationId:{in:locationIds}}});await prisma.location.deleteMany({where:{id:{in:locationIds}}});if(otherCompanyId)await prisma.company.delete({where:{id:otherCompanyId}});await prisma.$disconnect();
});
