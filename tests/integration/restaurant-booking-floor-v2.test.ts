...
test("Floor V2: due ordini concorrenti stesso tavolo hanno un solo vincitore",async()=>{const results=await Promise.allSettled([openOrder(companyId,locationA,userId,{tableId:table1,guestCount:2,serviceType:"DINE_IN"}),openOrder(companyId,locationA,userId,{tableId:table1,guestCount:2,serviceType:"DINE_IN"})]);const won=results.filter(x=>x.status==="fulfilled");assert.equal(won.length,1);if(won[0].status==="fulfilled")orderIds.push(won[0].value.id);for(const x of won)if(x.status==="fulfilled")await prisma.restaurantOrder.update({where:{id:x.value.id},data:{status:"CLOSED"}});await prisma.restaurantTable.update({where:{id:table1},data:{status:"AVAILABLE"}})});
test("Floor V2: prenotazione singola contro combinazione ha un solo vincitore",async()=>{const start=future(50,12),results=await Promise.allSettled([createReservation(companyId,null,randomUUID(),{locationId:locationA,guestName:"Single race",partySize:2,startTime:start,tableId:table1}),createReservation(companyId,null,randomUUID(),{locationId:locationA,guestName:"Combo race",partySize:4,startTime:start,tableIds:[table1,table2]})]);const won=results.filter(x=>x.status==="fulfilled");assert.equal(won.length,1);for(const x of won)if(x.status==="fulfilled")reservationIds.push(x.value.reservationId)});
test("Floor V2: promozione waitlist concorrente ha un solo vincitore",async()=>{const start=future(60,12);await prisma.restaurantTable.update({where:{id:table2},data:{status:"OUT_OF_SERVICE"}});const rows=await Promise.all(["A","B"].map(code=>prisma.restaurantReservation.create({data:{companyId,locationId:locationA,code:"WAIT-"+code+suffix,guestName:"Wait "+code,partySize:2,reservationDate:start,startTime:start,endTime:new Date(start.getTime()+3600000),status:"WAITLIST"},select:{id:true}})));reservationIds.push(...rows.map(x=>x.id));const results=await Promise.allSettled(rows.map(x=>transitionReservation(companyId,locationA,x.id,"CONFIRMED")));assert.equal(results.filter(x=>x.status==="fulfilled").length,1);await prisma.restaurantTable.update({where:{id:table2},data:{status:"AVAILABLE"}})});
test("Floor V2: buffer impedisce doppia occupazione",async()=>{const start=future(30,12);const first=await createReservation(companyId,null,randomUUID(),{locationId:locationA,guestName:"Buffer One",partySize:4,startTime:start,tableIds:[table1,table2]});reservationIds.push(first.reservationId);await assert.rejects(createReservation(companyId,null,randomUUID(),{locationId:locationA,guestName:"Buffer Two",partySize:2,startTime:new Date(start.getTime()+70*60000),tableId:table1}),/disponibile/)});
...
test("Floor V2: comanda libera rispetta lo stato fisico, non la colonna legacy",async()=>{
  // Lo stato fisico decide: DA RIASSETTARE e FUORI SERVIZIO bloccano.
  for(const physicalStatus of ["DIRTY","OUT_OF_SERVICE"] as const){
    await prisma.restaurantTable.update({where:{id:table1},data:{physicalStatus,status:"AVAILABLE"}});
  ...
  // La colonna legacy da sola non blocca più nulla.
  await prisma.restaurantTable.update({where:{id:table1},data:{physicalStatus:"READY",status:"DIRTY"}});
  const opened=await openOrder(companyId,locationA,userId,{tableId:table1,guestCount:2,serviceType:"DINE_IN"});
  orderIds.push(opened.id);
