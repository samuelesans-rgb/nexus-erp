...
import { assignTable, cancelReservation, confirmReservation, createReservation, newCancellationToken, RestaurantBookingError } from "../../lib/restaurant-booking";
...
async function create(key = randomUUID(), overrides: Partial<{ locationId: string; partySize: number; tableId: string; startTime: Date }> = {}) { const reservationStart = overrides.startTime ?? new Date(start.getTime() + reservationIds.length * 86_400_000); const result = await createReservation(companyId, userId, key, { cancellationToken: newCancellationToken(), locationId: overrides.locationId ?? locationA, guestName: "Booking Test", partySize: overrides.partySize ?? 2, startTime: reservationStart, tableId: overrides.tableId, source: "WEBSITE" }); reservationIds.push(result.reservationId); return result; }
